...
def run(video_path, face_path, model_weight, jitter, save_vis, display_off, save_text):
    cap = cv2.VideoCapture(1 if video_path is None else video_path)
    if not cap.isOpened(): return print("Error opening video")

    colors = list(Color("red").range_to(Color("green"), 10))
    ...
    while cap.isOpened():
        ...
        for box in bboxes:
            ...
            color = colors[min(int(score * 10), 9)].hex
    ...
    if eye_contact and start_time:  # final add
